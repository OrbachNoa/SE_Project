...
class ExclusionModel:
    ...
    # Set up the model by receiving the list of exam periods to manage.
    # We start by looking at the very first period (index 0).
    ...
    # Returns the actual period object we are currently working on
    @property
    def current_period(self) -> PeriodEditViewModel:
        return self._current_vm

    # Returns the position (number) of the period we are currently on (e.g., 0 for the first one)
    @property
    def current_index(self) -> int:
        return self._current_index

    # Tells us how many exam periods we have loaded in total
    @property
    def total(self) -> int:
    ...
    # Returns a list (technically a set) of all the dates the user has blocked off
    @property
    def excluded_dates(self) -> set[str]:
        return set(self._excluded_dates)

    # Checks if there is a previous period we can go back to
    def can_move_previous(self) -> bool:
        return self._current_index > 0

    # Checks if there is a next period we can move forward to
    def can_move_next(self) -> bool:
        return self._current_index < len(self._view_models) - 1

    # Updates the system's memory when the user picks a new start or end date
    ...
    # Saves our work on the current screen and goes back to the previous exam period
    ...
    # Saves our work on the current screen and advances to the next exam period
    ...
    # Takes the dates we have been editing in memory and locks them into the actual data model
    ...
    # Finalizes all changes across all periods and hands back the updated list of periods
