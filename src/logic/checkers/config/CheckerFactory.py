...
def build_checkers(config: ConstraintsConfig, courses: list,
    ...
    """Assembles the checker list for one scheduling run. The two base checkers
    are always present; each Phase-3 threshold checker is added only when its
    config value is set. Every checker's prepare() is then called once, so all
    precomputation happens in this single place inside the worker process.
    """
    ...
    if config is not None:
        if config.min_gap_obligatory is not None:
            checkers.append(MinDaysBetweenExamsChecker(GapScope.OBLIGATORY_ONLY, config.min_gap_obligatory))
        if config.min_gap_any is not None:
            checkers.append(MinDaysBetweenExamsChecker(GapScope.ANY, config.min_gap_any))
